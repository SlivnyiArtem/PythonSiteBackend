...
@pytest.mark.handler
def test_help_handler(test_mock_bot):
    test_mock_bot.application.message_handlers.handlers[0]()
    handlers.help_handler.assert_called_once_with(test_mock_bot.application)
...
def test_delete_money_recipient(test_mock_bot, mocker):
    ...
    message = MagicMock()
    handlers.delete_money_recipient(message, test_mock_bot)
    handlers.delete_money_recipient.assert_called_once_with(message, test_mock_bot)


def test_my_money_recipient(test_mock_bot, mocker):
    mocker.patch.object(handlers, 'my_money_recipient')
    message = MagicMock()
    handlers.my_money_recipient(message, test_mock_bot)
    handlers.my_money_recipient.assert_called_once_with(message, test_mock_bot)


def test_make_transaction(test_mock_bot, mocker):
    mocker.patch.object(handlers, 'make_transaction')
    message = MagicMock()
    handlers.make_transaction(message, test_mock_bot)
    handlers.make_transaction.assert_called_once_with(message, test_mock_bot)
